...
public class MinHeap<T extends Comparable<T>> extends Heap<T> {
    ...
    public void insert(T data) {
        ...
        int i = size;
        while (i > 0 && this.data.get(i).compareTo(this.data.get(parent(i))) == 1) {
            swap(i, parent(i));
            i = parent(i);
        }
        size++;
    ...
    public void heapify(int key) {
        ...
        int right = right(key);
        int smallest = key;
        if (left < size && this.data.get(left).compareTo(this.data.get(key)) == -1) {
            smallest = left;
        }
        if (right < size && this.data.get(right).compareTo(this.data.get(key)) == -1) {
            smallest = right;
        }
        if (smallest != key) {
            swap(smallest, key);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(h1);
        System.out.println("Size: " + h1.getSize());
        System.out.println("Fithull: " + h1.isFull());
